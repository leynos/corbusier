...
mod tests {
    ...
    use crate::message::domain::{
        ContentPart, ConversationId, Role, SequenceNumber, TextPart, ToolCallPart,
    };
    use mockable::DefaultClock;
    ...
    #[test]
    fn valid_message_passes_all_validation() {
        let validator = DefaultMessageValidator::new();
    ...
    fn empty_text_passes_with_lenient_config() {
        let validator = DefaultMessageValidator::with_config(ValidationConfig::lenient());
        let clock = DefaultClock;
        let message = Message::new(
            ConversationId::new(),
            Role::User,
