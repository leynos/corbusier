...
- Risk: Clock abstraction adds complexity
  Severity: low Likelihood: low Mitigation: Use mockable crate's Clock trait as
  documented in docs/reliable-testing-in-rust-via-dependency-injection.md.

- Risk: rstest-bdd (Behaviour-Driven Development (BDD)) learning curve
  Severity: medium Likelihood: medium Mitigation: Start with simple scenarios;
  refer to docs/rstest-bdd-users-guide.md for patterns.
...
- Decision: Use feature-based module layout (src/message/) rather than
  layer-based (src/domain/, src/ports/) Rationale: Aligns with AGENTS.md "group
  by feature, not layer" and hexagonal architecture skill guidance for smaller
  projects. Date/Author: Initial plan.
