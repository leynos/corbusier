...
use crate::in_memory::helpers::{clock, ctx, repo, runtime};
...
fn generation_across_conversations(
    ...
    let next1 = rt
        .block_on(repo.next_sequence_number(&ctx, conv1))
    ...
    rt.block_on(repo.store(&ctx, &msg1)).expect("store failed");
    ...
    let next_conv2 = rt
        .block_on(repo.next_sequence_number(&ctx, conv2))
        .expect("next_sequence_number failed");
    ...
    .expect("message creation failed");
    rt.block_on(repo.store(&ctx, &msg2)).expect("store failed");

    let conv1_messages = rt
        .block_on(repo.find_by_conversation(&ctx, conv1))
        .expect("find_by_conversation failed");
    let conv2_messages = rt
        .block_on(repo.find_by_conversation(&ctx, conv2))
        .expect("find_by_conversation failed");
