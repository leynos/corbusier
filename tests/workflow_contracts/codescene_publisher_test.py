...
def test_the_publisher_only_uploads_from_the_trunk() -> None:
    ...
    token. ``workflow_dispatch`` can name any branch, and the uploader checks
    neither ref nor event in upload mode, so without the ref test CodeScene
    would accept that branch's coverage as the trunk's. Nothing reports
    that, which is why it is asserted rather than trusted.
    """
    ...
    assert condition == EXPECTED_UPLOAD_CONDITION, (
        ...
        "load-bearing: the ref test stops a dispatch from another branch "
        "publishing that branch's coverage as the trunk's, and the "
...
def test_the_uploader_is_pinned_and_passes_no_retired_input() -> None:
    ...
    not asserted, so a Dependabot bump passes; a branch or tag name fails.
    """
    upload = _sole_upload()
...
def test_the_publisher_serializes_and_is_not_cancelled() -> None:
    ...
    overlap, and GitHub keeps a single pending run per group, so a newer
    trigger replaces an older pending run. No commit order is promised,
    since GitHub does not promise to start runs in trigger order. A manual
    re-run keeps its run id, so it republishes that commit's coverage but
    replaces no baseline saved under a run-keyed cache key. Cancelling
    instead would abandon a running upload and its baseline write.
    """
