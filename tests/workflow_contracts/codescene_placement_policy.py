...
#: The publisher's complete trigger set. ``workflow_dispatch`` stays because
#: merges made by the automerge workflow's ``GITHUB_TOKEN`` fire no push, so a
#: dispatch is the only way to publish them; the upload's ref guard below is
#: what stops a dispatch from another branch publishing that branch.
...
#: contract asserts a full commit SHA, never a specific one, so a Dependabot
#: bump does not fail the suite. The floor, shared-actions a5765019, whose
#: manifest pins the CodeScene CLI, is recorded in a comment beside the call
#: instead.
