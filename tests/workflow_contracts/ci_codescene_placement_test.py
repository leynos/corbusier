...
def test_no_pull_request_lane_calls_this_repository_by_ref(name: str) -> None:
    ...
    ``leynos/corbusier/.github/workflows/x.yml@main``. GitHub runs that
...
def test_the_pull_request_lane_still_ratchets_coverage() -> None:
    ...
    # where every check above still sees it, and runs it never. `ci.yml` runs
    # only for pull requests and dispatches, so the step has no reason to be
    # conditional at all, and any condition is refused.
    assert "if" not in generators[0], (
        f"{RATCHET_LANE}'s coverage step runs when "
        f"{generators[0].get('if')!r}; it must be unconditional. Any "
        "condition can disable the ratchet while leaving the step visible in "
        "the diff."
    )
