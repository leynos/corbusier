...
#: compilation through it. A repin to any of these leaves sccache
#: downloaded on every Rust job, caching nothing and reporting nothing,
#: and arrives looking like pins being brought up to date. Dependabot
#: proposed exactly that in corbusier #167, moving every reference 37
#: commits backwards to `57a33fa6`.
...
def coverage_jobs(
    ...
    for workflow, text in texts.items():
        ...
        for name, job in of_type(document.get("jobs"), dict).items():
            ...
            steps = [of_type(step, dict) for step in of_type(job_map.get("steps"), list)]
